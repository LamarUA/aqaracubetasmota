...
class AqaraCubeDriver
    ...
    def handle_zb_received(msg)
        ...
        if self.device_addr && msg.contains(self.device_addr)
            ...
            # Publish result wrapped in AqaraCube object
            if size(result) > 0
                var wrapped_result = {"AqaraCube": result}
                print("DEBUG: Publishing result:", json.dump(wrapped_result))
                tasmota.publish_result(json.dump(wrapped_result), 'RESULT')
                return true
            else
